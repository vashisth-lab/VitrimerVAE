...
def load_model(args):
    ...
    for i in range(args.epoch):
        if os.path.exists('%s/ckpt/prop%d.model' % (args.savedir, i)):
            start += 1

    if start == 0:
        model_state, optimizer_state, _ = torch.load('%s/ckpt/%s' % (args.savedir, args.model))
        model.load_state_dict(model_state)
        optimizer.load_state_dict(optimizer_state)
    else:
        model_state, optimizer_state, scheduler_state = torch.load('%s/ckpt/prop%d.model' % (args.savedir, start - 1))
        model.load_state_dict(model_state)
        optimizer.load_state_dict(optimizer_state)
...
def main(args):
    ...
    for epoch in range(start, args.epoch):
        ...
        dataset = DataFolder_prop(seed=epoch)

        log = open('%s/ckpt/prop.log' % (args.savedir), 'a')
        ...
        for it, batch in enumerate(dataset):
            ...
            batch_prop = batch[5]
            model.zero_grad()
            loss, kl_div, loss_aci, loss_epo, loss_prop, wacc_aci, iacc_aci, tacc_aci, sacc_aci, wacc_epo, iacc_epo, tacc_epo, sacc_epo = model((batch_aci, batch_epo, batch_prop), beta=beta, beta_prop=args.beta_prop, prop=True)
            ...
            if (it + 1) % args.print_iter == 0 or (it + 1) == total_iter:
                meters /= meters_cnt
                log.write("[%d] beta: %f, beta_prop: %.3f, loss: %.3f, KL: %.2f, loss_aci: %.2f, loss_epo: %.2f, loss_prop: %.2f, Word_aci: %.2f, %.2f, Topo_aci: %.2f, Assm_aci: %.2f, Word_epo: %.2f, %.2f, Topo_epo: %.2f, Assm_epo: %.2f\n" % 
        ...
        torch.save(ckpt, '%s/ckpt/prop%d.model' % (args.savedir, epoch))

        with open('%s/train_loss_joint.csv' % (args.savedir), 'a') as f:
        ...
        with torch.no_grad():
            ...
            with open('%s/test_loss_joint.csv' % (args.savedir), 'a') as f:
                    f.write('%d,%d,%.4f,%.4f,%.4f,%.4f\n' % 
