...
def main(args, seed, acid_init, epoxide_init):
    ...
    tensor_aci, tensor_epo = tensorize([acid_init], [epoxide_init], args.vocab_aci, args.vocab_epo)
    ...
    for _ in range(30):
        z = []
        for _ in range(50):
            z1_sample = np.random.normal(0, 1, size=z1.shape)
            z.append(np.concatenate((z1_sample, z2, z3), axis=1))
        z = np.vstack(z)
        z_tensor = torch.tensor(z).float().cuda()
        aci_dec, epo_dec = model.decode(z_tensor)
        for i in range(50):
    ...
    for i in range(args.latent_size):
        d['z' + str(i)] = z[:, i]
    df = pd.DataFrame(d)
    df.to_csv('%s/bo_initial_acid_%d.csv' % (args.savedir, seed), index=False)
...
if __name__ == "__main__":
    ...
    # provide acid just to make the shared part of latent vector
    # acid-specific part will be perturbed
    acid = 'O=C(O)CCCCC(=O)O'
    epoxide = 'CC(C)(c1ccc(OCC2CO2)cc1)c1ccc(OCC2CO2)cc1'
    for i in range(1, 11):
        if not os.path.isfile('%s/bo_initial_acid_%d.csv' % (args.savedir, i)):
            main(args, i, acid, epoxide)
